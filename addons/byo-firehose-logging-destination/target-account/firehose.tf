...
data "aws_iam_policy_document" "firehose_policy" {
  ...
  statement {
    ...
    resources = [
      for name in keys(var.log_destinations) : "arn:aws:logs:${data.aws_region.current.id}:${data.aws_caller_identity.current.account_id}:log-group:/aws/kinesisfirehose/${var.log_destinations[name].name}:*"
    ]
  }
