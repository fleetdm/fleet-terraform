...
This addon no longer manages an S3 bucket policy on the software installers bucket. The bucket policy is now managed by the core module (`byo-ecs`) when `software_installers.attach_deny_insecure_transport_policy = true`. When `cloudfront_distribution_arn` is also set, the core module automatically includes the CloudFront allow statement in that policy.
