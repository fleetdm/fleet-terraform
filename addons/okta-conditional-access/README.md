...
By default the target group (`tg-0`) speaks HTTP on port 80 to the Fleet backend. If your Fleet deployment terminates TLS (e.g. `enable_fleet_server_tls = true`, serving HTTPS on 8080), pass a `backend` object so the target group speaks HTTPS instead. The health check protocol follows the target group protocol automatically.

```hcl
module "okta-conditional-access" {
  ...
  alb_config = merge({
    name            = "${local.customer}-okta"
    subnets         = module.fleet.vpc.public_subnets
    certificate_arn = module.okta_acm.acm_certificate_arn
    trust_store     = { ... }
  }, local.enable_fleet_server_tls ? {
    backend = { protocol = "HTTPS", port = 8080 }
  } : {})
}
```
